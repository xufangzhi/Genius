...
from dpo_utils import dpo_loss, ipo_loss, ropo_loss, rpo_loss, simpo_loss, advantage_weighted_dpo_loss, advantage_weighted_rejectratio_dpo_loss, advantage_weighted_rejectratio_with_average_dpo_loss, concatenated_forward, DataCollatorForSeq2SeqDPO
from dpo_utils import advantage_weighted_chosenrejectratio_with_average_dpo_loss
...
def parse_args():
    ...
    parser.add_argument(
        "--has_weights", action="store_true", help="Whether has weights in the input"
    )
    parser.add_argument(
        "--checkpointing_steps",
        type=str,
...
def encode_with_messages_format(example, tokenizer, max_seq_length, has_weights=False):
    '''
    ...
    if has_weights:
        chosen_weights = example['chosen_weights']
        rejected_weights = example['rejected_weights']
        chosen_average_weights = example['chosen_average_weights']
    ...
    if has_weights:
        return {
            ...
            'chosen_average_weights': chosen_average_weights,
...
def encode_dpo_example(example, tokenizer, max_seq_length, has_weights=False):
    """
    Here we assume each example has a rejected and chosen field, both of which are a list of messages.
    Each message is a dict with 'role' and 'content' fields.
    We assume only the last message is different, and the prompt is contained in the list of messages.
...
def main():
    ...
    with accelerator.main_process_first():
        lm_datasets = raw_datasets["train_prefs"].map(
            ...
            remove_columns=[name for name in raw_datasets["train_prefs"].column_names if name not in ["chosen_input_ids", "chosen_labels", "chosen_attention_mask", "chosen_weights", "rejected_input_ids", "rejected_labels", "rejected_attention_mask", "rejected_weights", "chosen_average_weights", "rejected_average_weights"]],
    ...
    for epoch in range(starting_epoch, args.num_train_epochs):
        ...
        for step, batch in enumerate(active_dataloader):
            ...
            with accelerator.accumulate(model):
                # print(batch)
                average_policy_chosen_logps, average_policy_rejected_logps, policy_chosen_logps, policy_rejected_logps, chosen_weights, rejected_weights, average_weights = concatenated_forward(model, batch, has_weights)
                with torch.no_grad():
                    if args.use_lora:
                        with accelerator.unwrap_model(model).disable_adapter():
                            average_reference_chosen_logps, average_reference_rejected_logps, reference_chosen_logps, reference_rejected_logps, chosen_weights, rejected_weights, average_weights = concatenated_forward(model, batch, has_weights)
                    else:
